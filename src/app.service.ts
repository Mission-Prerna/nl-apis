...
export class AppService {
  ...
  private monthQuarterMap: Record<string, string> = {
    '1': 'jan_mar',
    '2': 'jan_mar',
    '3': 'jan_mar',
    '4': 'apr_jun',
  ...
  async createAssessmentVisitResult(
    ...
    try {
      ...
      const assessmentVisitResult = await this.getAssessmentVisitResult(
        tables,
      ...
      const response = await this.insertAssessment(
        createAssessmentVisitResultData,
        uniqueStudents,
        totalTimeTaken,
        tables,
        assessmentVisitResult,
      );
      ...
      if (createAssessmentVisitResultData.actor_id === ActorEnum.EXAMINER) {
        await this.checkExaminerSchoolResultReCalculation(
          createAssessmentVisitResultData,
        );
      }
  ...
  private async insertAssessment(
    ...
    const response = await this.prismaService.$transaction(
      async (tx) => {
        ...
        await this.handleNonODKAssessments(
          createAssessmentVisitResultData,
          uniqueStudents,
          totalTimeTaken,
          tables,
  ...
  private async getAssessmentVisitResult(
    tables: TypeAssessmentQuarterTables,
    createAssessmentVisitResultData: CreateAssessmentVisitResult,
  ) {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const assessmentVisitResult = await this.prismaService[
  ...
  private async handleNonODKAssessments(
    createAssessmentVisitResultData: CreateAssessmentVisitResult,
    uniqueStudents: Record<string, number>,
    totalTimeTaken: number,
    tables: TypeAssessmentQuarterTables,
  ...
  private async checkExaminerSchoolResultReCalculation(
    ...
    const mentorMapping =
      await this.prismaService.assessment_cycle_district_mentor_mapping.findFirst(
        {
          select: {
            cycle_id: true,
          },
    ...
    const schoolResult =
      await this.prismaService.assessment_cycle_school_nipun_results.findFirst({
        where: {
          mentor_id: createAssessmentVisitResultData.mentor_id,
          udise: createAssessmentVisitResultData.udise,
  ...
  async getMentorDetails(
    ...
    const schoolsList = await this.getMentorSchoolListIfHeHasVisited(
      mentor,
      month,
      year,
    );
    return {
      mentor: mentor,
      school_list: schoolsList.length
        ? schoolsList
        : examinerCycleDetails
        ? examinerCycleDetails?.schools_list
        : [],
  ...
  async getMentorBots(mentorId: bigint) {
    ...
    and mentor_segmentation.mentor_id = ${mentorId}`.then(
      (response: Array<segment_bots>) =>
