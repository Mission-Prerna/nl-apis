...
export class SchoolService {
  ...
  async getSchoolStudentsResults(
    ...
    const query = `
      ...
      FROM (
        ...
        } avr2 ON (avrs.assessment_visit_results_v2_id = avr2.id and avr2.udise = ${udise} and avr2.actor_id = ${
      ActorEnum.TEACHER
    })
        WHERE
    ...
    for (const grade of grades) {
      response.push({
        ...
        period: this.i18n.t(
          `months.${moment(month, 'M').format('MMMM')}Month`,
          { lang: lang },
        ),
        summary: [
          {
  ...
  async getSchoolTeacherPerformance(mentor: Mentor, udise: number) {
    ...
    const tablesForFirstDate = this.appService.getAssessmentVisitResultsTables(
      firstDate.getFullYear(),
      firstDate.getMonth() + 1,
    );
    const tablesForLastDate = this.appService.getAssessmentVisitResultsTables(
    ...
    if (responseSecondTable) {
      ...
      weekly = {
        ...
        updated_at:
          (responseFirstTable?.updated_at || 0) +
          (responseSecondTable?.updated_at || 0),
        assessed_student_ids: (responseFirstTable?.assessed_student_ids || '')
          .split(',')
          .concat((responseSecondTable?.assessed_student_ids || '').split(','))
          .join(','),
        nipun_student_ids: (responseFirstTable?.nipun_student_ids || '')
          .split(',')
          .concat((responseSecondTable?.nipun_student_ids || '').split(','))
          .join(','),
      };
    }
    ...
    const currentMonthTables = this.appService.getAssessmentVisitResultsTables(
      currentYear,
      currentMonth,
    );
    const currentMonthStats =
      await this.getTeacherStudentsSummaryResultForPeriod(
        currentMonthTables,
        mentor,
        udise,
    ...
    const lastMonthTables = this.appService.getAssessmentVisitResultsTables(
      lastYear,
      lastMonth,
    );
